...
| Target Persona Fit / Decision-maker Role Fit | 0–25 | CEO, Founder, CTO, or COO |
...
**Initial PTB status — two sequential lifecycle stages, not one placeholder state.** ICP Score is calculated once, at sourcing/qualification, using the criteria above. Buying-propensity scoring now has two stages, owned entirely by `ptb-scoring`: an **Initial Buying Signal Score** (pre-engagement, calculated by `ptb-scoring` Path A during Scheduler 1, from company/market evidence observable before any outreach — this is a distinct model from the four criteria below, documented in `ptb-scoring`'s own Path A section, not restated here) and the **Post-Engagement PTB Score** described by the four criteria immediately above (Engagement, Urgency signal, Multi-threading, Decision-maker confirmed reachable) — this second score is **not** calculated at initial sourcing, because none of those four criteria can be evidenced before any engagement has occurred. A newly sourced prospect's initial Post-Engagement PTB status is therefore **N/A / Not Yet Scored** for that specific score, not a low or zero score — this no longer means the prospect has no buying-propensity signal at all, only that the post-engagement one hasn't been calculated yet. The Post-Engagement PTB Score may be calculated, or re-scored, only once applicable engagement evidence exists — for example an email reply, a LinkedIn response or connection acceptance where applicable, a meeting or call, a stated urgency/timeline, multi-threading, or confirmed decision-maker responsiveness — using the same four criteria and 0–100 structure above, and it then supersedes the Initial Buying Signal Score in Zoho's `Twitter` field; the two scores are never added, averaged, or combined. No additional Post-Engagement PTB criteria are introduced by this note.

**ICP Buying Signal vs. PTB Urgency Signal — do not confuse these.** ICP Buying Signal (the "Buying signal present" row in ICP Score above) = observable company-level signal available before outreach, such as funding, hiring, acquisition, product launch, expansion, or similar external/company evidence. PTB Urgency Signal (the "Urgency signal" row in PTB Score above) = prospect-level buying evidence revealed through engagement, such as a stated timeline, active project need, budget confirmation, request for proposal, or equivalent direct buying evidence. ICP Buying Signal is pre-outreach company evidence; PTB Urgency Signal is post-engagement buying evidence — they score different things at different stages and must not be conflated.

**Approved operational PTB model.** Engagement 0–40, Urgency Signal 0–30, Multi-threading 0–15, Decision-maker Confirmed Reachable 0–15 — total 100 points, per the criteria table above. This model is approved for operational use and applies to post-engagement PTB scoring and re-scoring.

**Known inconsistency — DATA CONFLICT, not resolved here.** An internal Sales Bible reference defines a single blended scoring model with different weights (Company fit 0–20, Buying signal 0–25, Persona 0–20, Geography 0–15, Engagement 0–20). The two-score split above is used for AI agent purposes, because engagement data does not exist at sourcing time and cannot be scored then — but this document does not have the authority to unilaterally resolve which model is correct, and does not claim to permanently supersede the Sales Bible model. Flag this if a human references the Sales Bible weights, and escalate for confirmation rather than silently picking one. **Status: DATA CONFLICT / HUMAN CONFIRMATION REQUIRED — open, pending leadership decision.**

**Independent scores — do not combine.** ICP Score and PTB Score are independent 0–100 scores. They must never be added, averaged, multiplied, weighted together, or otherwise combined into a single score. ICP Score determines ICP qualification. PTB Score determines engagement/buying propensity after applicable engagement signals exist. The 0–100 tier bands below apply independently to each score.

### Tier bands — applied independently to ICP Score and to PTB Score
...
Sender/mailbox assignment for this persona is governed by `context/campaigns.md` (LIVE CAMPAIGN REGISTRY section), not by this document.
...
Sender/mailbox assignment for these personas is governed by `context/campaigns.md` (LIVE CAMPAIGN REGISTRY section), not by this document.
...
Sender/mailbox assignment for any persona below is governed by `context/campaigns.md` (LIVE CAMPAIGN REGISTRY section) — the "Sender" column is an indicative reference only, not the authoritative assignment.

| Role | Goals | Challenges | KPIs | Buying Motivation | Preferred Messaging | Sender (indicative — see `campaigns.md`) |
|---|---|---|---|---|---|---|
...
| Last verified | This revision, created from `migration/original-source/icp.md` and audited against current governance |
...
4. **PTB Score component weights** — Resolved. Approved for operational use as the post-engagement PTB model (Engagement 0–40, Urgency Signal 0–30, Multi-threading 0–15, Decision-maker Confirmed Reachable 0–15, total 100). See "Approved operational PTB model" in the PTB Score section above; no longer an open item.
...
4. Never enrich a prospect with an ICP Score below 50. Neither buying-propensity score gates the enrichment decision: the Post-Engagement PTB Score is N/A until engagement evidence exists, and the Initial Buying Signal Score is not calculated until after this enrichment step (see `ptb-scoring` Path A, invoked later in the pipeline) — gating enrichment on either wastes Apollo credits on leads that will not be sequenced, or blocks enrichment on a score that doesn't exist yet.
