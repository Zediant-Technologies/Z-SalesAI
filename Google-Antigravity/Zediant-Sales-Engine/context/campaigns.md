...
| **Email-Only channel execution** | Outreach is focused 100% on cold email deliverability and sequencing. Manual LinkedIn touches are postponed during this ramp-up phase to maintain high pipeline velocity. |
| **Conservative volume pacing** | Daily sending cap is **25–35 emails/day total** across all sends (initial + follow-ups) from the active mailbox (`rajeev@zedianttechnologies.info`). Enrol approximately **7–10 high-intent new leads/day** to prevent follow-up queues from backing up. |
| **Pacing delay** | Minimum randomized interval of **300–480 seconds (5–8 minutes)** between outbound emails to protect sender domain reputation. |
...
| **Zero tracking pixels** | Both open tracking and click tracking must remain **disabled** (`enable_open_tracking: false`, `enable_click_tracking: false`). Plain text emails without redirect links yield 15–25% higher inbox placement. |
| **Threaded sequence cadence** | Steps 2 and 3 must send as threaded replies (`Re: {{Subject}}`) directly in the same conversation thread. Threaded bumps look human; disconnected subject lines trigger spam filters. |
...
| **Personalise the first line** | Never send a fully generic template. Enforce `context/zediant_outreach_natural_language_blacklist.md`. |
...
| # | Campaign Name | Sequence Structure | Cadence & Threading |
|---|---|---|---|
| **C1** | AI-Enabled Product Engineering | **5-step email** | Step 1 (Day 1) → Step 2 (Day 3, Threaded) → Step 3 (Day 6, Threaded) → Step 4 (Day 9) → Step 5 (Day 13 Breakup) |
| **C2** | Engineering Pods & Staff Augmentation | **5-step email** | Step 1 (Day 1) → Step 2 (Day 3, Threaded) → Step 3 (Day 6, Threaded) → Step 4 (Day 9) → Step 5 (Day 13 Breakup) |
| **C3** | Platform Engineering & Cloud Modernization | **5-step email** | Step 1 (Day 1) → Step 2 (Day 3, Threaded) → Step 3 (Day 6, Threaded) → Step 4 (Day 9) → Step 5 (Day 13 Breakup) |
| **C4** | Middleware & API Integration (ZCoupler) | **5-step email** | Step 1 (Day 1) → Step 2 (Day 3, Threaded) → Step 3 (Day 6, Threaded) → Step 4 (Day 9) → Step 5 (Day 13 Breakup) |
| **C5** | Enterprise Custom Development & Modernization | **5-step email** | Step 1 (Day 1) → Step 2 (Day 3, Threaded) → Step 3 (Day 6, Threaded) → Step 4 (Day 9) → Step 5 (Day 13 Breakup) |

*Volume Allocation:* All campaigns share the single active mailbox capacity during ramp-up (**25–35 emails/day total**). Sourcing pulls ~7–10 leads/day allocated based on **High-Intent Signal Priority** (where technical need and PTB score are strongest). Sequence IDs are resolved dynamically by `saleshandy-distribution` via title match at push time.

## ⚠️ Open items — operational status

### 1. Mailbox warm-up status
Active mailbox (`rajeev@zedianttechnologies.info`) Warmbox warmup concludes on **September 17, 2026** (currently >89% complete). Live sequences may be activated on September 17.

### 2. Send schedule timezone
Confirm schedule directly in Saleshandy before activation:
