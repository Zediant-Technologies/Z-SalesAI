...
Saleshandy is the active outbound email platform.

A prospect may be added to a Saleshandy sequence only when the required
outreach approval state has been achieved.
...
Do not send directly from Apollo.

Do not bypass Zoho approval.

---

# 16. SALESHandy CAMPAIGN MAPPING

Do not hard-code business campaign logic around Saleshandy sequence IDs.
...
If the required Saleshandy sequence mapping does not exist:
...
Follow the current Saleshandy campaign configuration.
...
Where Saleshandy creates a LinkedIn task that requires human
execution:
...
Before a prospect is sent to Saleshandy, verify:
...
[ ] Saleshandy sequence identified
