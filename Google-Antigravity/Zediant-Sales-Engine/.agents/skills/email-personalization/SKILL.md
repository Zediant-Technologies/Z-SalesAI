...
Requirements: 1-2 sentences, approximately 15-25 words, specific to the prospect/company, based on the strongest credible trigger, natural and conversational, starts with prospect/company context. Do not start with generic compliments. Do not mention Zediant. Do not include the main sales pitch, the signature, the main CTA, or a direct repeat of the pain point. **No AI-typical special characters** (em dashes, en dashes, curly quotes used decoratively, etc.) — write it the way a person would actually type it.

Apply `zediant_outreach_natural_language_blacklist.md` (Step 0) while drafting, not after: focus the observation on product activity, expansion, hiring, new features, new markets, engineering activity, or a technology initiative, per that file's Rules for the Four Zoho Fields, and avoid its Corporate/Marketing, AI-buzzword, and generic-growth-statement language. Only use "AI" if the prospect's own verified activity makes it genuinely relevant — never because the campaign happens to be C1.

**Second person only — never refer to the recipient in third person by their own first name.** The live Saleshandy template greets the recipient directly (`Hi {{First Name}},`) immediately before this field is inserted. Content that then refers back to that same person in third person — "Jason's team," "I'd guess Nicki is probably feeling," "under Mahesh's leadership" — reads as if the email is describing the recipient to someone else rather than talking to them, breaking the moment it's read in context. Always address the recipient as "you/your," and reserve third person strictly for the company itself ("{company}'s platform footprint has grown") or for a different named person (e.g., a hiring lead who isn't the recipient). This applies to `Email_Pain_Points` on the same logic wherever it addresses the recipient directly, and to the LinkedIn connect/follow-up messages, which already speak in second person by convention — the failure mode is specific to `Email_Personalised_Opening` because it's the field most tempted to open with "[Company]'s [Name] has been..." styled scene-setting.

*Wrong:* "With Snug.com's integration surface expanding, I'd guess Jason's team is spending more time than they'd like wiring up APIs rather than shipping product." *Right:* "With Snug.com's integration surface expanding, I'd guess your team is spending more time than you'd like wiring up APIs rather than shipping product." (This exact bug shipped across all five campaign templates in a live batch on August 13, 2026, caught only when a human read the field inside the actual template rather than in isolation — see Validation below, which now requires the same check before output ever leaves this skill.)
...
Requirements: 1-2 sentences, approximately 15-25 words, must logically follow the Opening (don't repeat it), focus on the business/engineering challenge, no unsupported claims, no exaggeration, don't mention Zediant unless the live Saleshandy template specifically requires it. **No AI-typical special characters.**
...
- Each of the four fields reads naturally aloud, avoids unnecessary repetition of another field, fits the exact live-template slot it's inserted into (see Saleshandy Template Compatibility), and avoids unsupported claims and promotional or sales-heavy language
...
- **Preserved unchanged:** the absolute never-fabricate-a-case-study rule, all LinkedIn CONNECT/FOLLOW-UP content and word limits (20-45/55 max, 40-70), the second-person requirement, the em-dash/AI-phrasing/AI-framing bans, the confidentiality-tier rules, Saleshandy Template Compatibility, LinkedIn Selection Strategy (Section 4), and every unrelated section. This was a file-only edit — no Zoho, Apollo, Saleshandy, or Cliq activity occurred as part of making this change.
...
- **No changes to**: ICP scoring, Initial Buying Signal Score, Post-Engagement PTB, campaign selection or the C1-C5 taxonomy, campaign/sender allocation, Apollo search logic or credit rules, deduplication, lead qualification, CRM field mappings or write logic, Scheduler 1/2, Saleshandy, Case Study logic, Email channel personalization rules, Cliq rules, BDM approval rules, or Section 4's LinkedIn Selection priority ranking. This was a file-only edit — no Apollo, Zoho, Saleshandy, or Cliq activity occurred as part of making this change.
...
- Added a cross-reference in Saleshandy Template Compatibility explaining *why* this only surfaces when a field is read inside the template structure, not on its own
...
- Replaced stale Instantly references with Saleshandy, and List B/List C campaign references with C1-C5, to match `campaign-selection` v2.0 and `crm-update` v4.0
