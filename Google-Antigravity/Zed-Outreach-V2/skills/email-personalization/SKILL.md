...
## Step 8 — Generate Apollo "Personalised Email" Body (Body Only)

Apollo custom field: `Personalised Email` (id: `6aa79177f203040018e0af9d`), mirrored to Zoho field: `Email_Personalised_Opening`.

**The Architectural Purpose:** In the prior platform, Step 1 awkwardly stitched separate snippets (`{{Email Personalised Opening}} {{Email Pain Points}}` + static spintax). This created visible seams and risk of robotic cadence. In Apollo, Google Antigravity crafts the **`Personalised Email` custom field as a complete, cohesive first-step email body** (approx. 50–90 words) that flows seamlessly from opening observation to low-friction CTA.

> [!IMPORTANT]
> ### EMAIL BODY ONLY RULE (CRITICAL)
> The `Personalised Email` field MUST ONLY contain the **Email Body** (the middle narrative paragraphs).
> - **DO NOT include greetings or salutations** (e.g. NEVER start with `Hi {{first_name}},`, `Hello John,`, or `Dear [Name],`). The salutation `Hi {{first_name}},` is permanently embedded at the top of the Apollo Step 1 sequence template.
> - **DO NOT include sign-offs or signatures** (e.g. NEVER end with `Best,`, `Regards,`, `Cheers,`, `Rajeev Jaiswal`, or `Zediant Technologies`). The sign-off and sender signature are permanently embedded at the bottom of the Apollo Step 1 sequence template.
> - Including greetings or sign-offs inside this field will cause duplicate salutations and signatures in live prospect inboxes. Start immediately with the observation/trigger sentence, and end with the low-friction CTA question.

**The 3-Beat Micro-Structure (Visual Spacing Standard):**
Never output the email body as a single unbroken wall of text. Over 65% of executive readers scan emails on mobile phones in 3 seconds. The `Personalised Email` custom field must ALWAYS be formatted into **3 distinct visual beats separated by double newlines (`\n\n`)**:

1. **Beat 1: The Observation / Hook (Standalone paragraph, 1 sentence):**
   Opens directly with the verified prospect/company trigger and MUST BE PRODUCT-GROUNDED. 
   - Research and cite the prospect's actual flagship product/platform name (e.g. `Crunchwork`, `Hutly`, `MachShip`, `Cloud Assess`) and the specific workflow it solves (e.g. `streamlining claims and contractor workflows`).
   - NEVER use vague, generic consulting filler like `"focus on modernizing user workflows"`, `"digital transformation"`, or `"expanding platform capabilities"`. If a real engineer looked at the company, they'd name the product.
   *Example:*
   `Saw what you guys are building with Crunchwork and how you're streamlining claims and contractor workflows.`

2. **Beat 2: The Operational Pressure & Pod Solution (1–2 sentences):**
   Transitions smoothly to the delivery bottleneck and articulates Zediant's core differentiator matching the specific campaign:
   - **For C1 (AI-Enabled Product Engineering):** Position Zediant as **AI-Accelerated Delivery & Ownership**. Embed dedicated senior engineering squads in 2-3 weeks to take full architectural ownership of complex feature modules, shipping 30-40% faster using modern AI-assisted engineering workflows. (Avoid drifting into generic "extra hands" or "staff aug" language).
   - **For C2 (Engineering Pods / Capacity):** Position Zediant as rapid sprint capacity (senior fullstack developers joining your repo, standups, and sprint rituals within 14 days without agency overhead).
   - **For C3 (Platform & Cloud):** Position Zediant as senior platform/cloud squads hardening architecture and optimizing throughput without disrupting active product sprints.
   - **For C4 (Middleware & API):** Position Zediant as integration specialists building resilient bi-directional data pipelines and API layers under senior architectural ownership.
   - Avoid generic AI formulaic clichés like `"Scaling X while maintaining Y usually puts heavy pressure on Z"`. Write naturally peer-to-peer (e.g. `"pulls senior engineers in two directions"`).
   *Example (C1 - Pragmatic & Natural):*
   `Rolling out partner APIs while keeping the core rating engine fast usually pulls senior engineers in two directions. We embed senior squads in 2-3 weeks to take complete architectural ownership of feature modules, using modern tooling to ship about 35% faster.`

3. **Beat 3: The Standalone Permission-Based Ask (Standalone paragraph, 1 sentence):**
   Closes with a single permission-based question standing completely on its own line for maximum visual contrast.
   - Frame the ask around offering a 2-minute teardown of how we helped a similar product team accelerate release cycles by 35% without lengthy hiring lags.
   *Example:*
   `Open to checking a 2-minute teardown of how we helped a similar product team accelerate release cycles by 35%?`

**Hard Style Rules:**
- **Second person only:** Address the recipient as "you / your team". Never use third person about the recipient ("Curtis's team") after the template greeted them directly.
- **No AI special characters:** Strictly NO em dashes (`—`), NO en dashes (`–`), NO double hyphens (`--`), NO decorative quotes. Use plain commas, periods, or standard hyphens.
- **Natural language blacklist:** Must strictly satisfy `context/playbooks/_blacklist-digest.md`.
- **Human typing test:** Must read as if Rajeev personally typed this specific email to one CTO or VP Engineering.

### Multi-Step Cadence: How Custom Fields Power the 6-Touch Sequence
Antigravity enriches multiple custom fields on the Apollo contact that dynamically power the 6-touch cadence across Email and LinkedIn:
- **Touch 1 (Day 1 - Auto Email):** Powered by `{{Personalised Email}}` (custom 3-beat email body).
- **Touch 2 (Day 3 - LinkedIn Connect Task):** Powered by `{{Company Trigger}}` (e.g. *"Hi {{First Name}} - noticed {{Company Trigger}} at {{Company}}... Would love to connect."*).
- **Touch 3 (Day 6 - Auto Threaded Email):** Threaded follow-up referencing `{{Pain Point}}` (e.g. *"Saw earlier how {{Company}} was navigating {{Pain Point}}... sharing a quick perspective on how we structure delivery."*).
- **Touch 4 (Day 9 - LinkedIn Casual InMail/DM Task):** Non-blocking casual follow-up without pitching or repeating `{{Pain Point}}`.
- **Touch 5 (Day 13 - Auto Threaded Email):** Threaded technical framework / 14-day production readiness benchmark asset.
- **Touch 6 (Day 17 - Auto Threaded Email):** Clean, professional breakup message leaving the door open.

*(Note: We also populate `Email_Pain_Points` [Step 9] and `Case_Study` [Step 7] because they are stored in Zoho CRM upon response and mirrored to Apollo).*
