...
**Campaigns are now C1-C5 only.** These are the actual Apollo sequences with documented limits, live in Apollo as of the migration from Instantly. Sender/mailbox assignment is not tracked by this skill or by `campaigns.md` — per explicit human instruction, which email account sends a given campaign is an operational Apollo configuration detail, not a campaign-selection concern. Campaign selection is **service-based**: the question is which C1-C5 service the prospect most plausibly needs, based on their actual business problem/technical need, business signal, ICP fit, PTB/buying signal, and persona — evaluated together as a weighted evidence score, not resolved by any single attribute (industry, company type, lead source, or persona) acting alone. The **output is always a C1-C5 campaign name** — the ones that actually exist. The live Apollo sequence is resolved by `apollo-distribution` at push time (by campaign name/title, never by an ID tracked in this skill) — this skill's job stops at naming the right C1-C5 campaign, not at tracking its sequence identity.
...
| Resolving which live Apollo sequence a campaign name maps to | `apollo-distribution` skill |
| Activating a campaign, changing its status, or adding a lead to Apollo | Nobody, automatically. Human approval required before outreach |
...
- **Do not output a Apollo sequence ID.** Sequence resolution by campaign name/title is owned entirely by `apollo-distribution`; this skill never stores or repeats a sequence ID.
...
- [ ] No Apollo sequence ID appears anywhere in the output
...
# CAMPAIGN REGISTRY (C1-C5) & APOLLO CUSTOM FIELD MAPPING

| Campaign | Target Segment (Apollo Custom Field) | Outreach Angle (Apollo Picklist) | Daily Limit | Primary Use Case |
|---|---|---|---|---|
| **C1** | `C1 - AI-Enabled Product Engineering` | `Product Development` | 30 | SaaS CTOs; product roadmap scaling |
| **C2** | `C2 - Engineering Pods & Staff Augmentation` | `Engineering Capacity` | 30 | Agencies; SaaS scaling; hiring gaps |
| **C3** | `C3 - Platform Engineering & Cloud Modernization` | `Engineering Capacity` | 30 | Platform/cloud/DevOps modernization |
| **C4** | `C4 - Middleware & API Integration (ZCoupler)` | `Other` | 30 | Integration, middleware, data flows |
| **C5** | `C5 - Enterprise Custom Development & Modernization` | `Product Development` | 30 | Enterprise features, legacy modernization |

Sender/mailbox assignment per campaign is intentionally not tracked here — per explicit human instruction (September 2026), which account sends a campaign is an Apollo configuration detail this skill and `campaigns.md` no longer encode. `apollo-distribution` resolves live sending configuration directly in Apollo at push time.

This skill outputs `Target Segment` and `Outreach Angle` for direct population into Apollo Contact custom fields (`6aa790d821b4e6001cb70994` and `6aa79157e03659000e5b1889`). `apollo-distribution` is solely responsible for resolving the current live Apollo sequence by matching the campaign's name/title at push time — no ID is duplicated here.
...
| `apollo-distribution` | Downstream. Resolves the selected campaign name/title to the current live Apollo sequence; owns all sequence-ID resolution |

The normal workflow is: Lead qualification → ICP Score → PTB Score (Initial Buying Signal) → Campaign selection → Personalization → CRM → BDM approval → Apollo. Campaign selection never activates outreach and never bypasses BDM approval.
...
| Apollo sequence IDs appearing in this skill's output | Zero |
...
**Change in 3.1 — sender/mailbox assignment removed (SURGICAL, EDIT-ONLY):** Per explicit human instruction, this skill no longer tracks or surfaces per-campaign sender mailbox assignment — it does not matter to the business which account sends which campaign, and that is a Apollo configuration detail, not a campaign-selection concern. Removed the "Sender is the documented campaign-level alias only" decision rule, the sender-identity-resolution Human Review Condition, the sender validation checklist line, and the "Sender (documented alias)" column from the Campaign Registry table (daily limits and use-case notes unchanged). `apollo-distribution` owns actual sending configuration. No campaign taxonomy, scoring model, weight, or threshold changed.
