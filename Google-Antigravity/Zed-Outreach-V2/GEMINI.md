...
Lead Acquisition
→ Qualification & ICP Scoring
→ PTB Evaluation & Company Trigger
→ Campaign Selection & Target Segment / Outreach Angle
→ Antigravity Personalization (Personalised Email + Pain Point + LinkedIn)
→ Apollo Contact Custom Fields Enrichment (Approval Status = "New", Zoho Sync Status = "Not Synced")
→ BDM Human Approval in Apollo (Approval Status = "Approved for Outreach")
→ Apollo Outreach (Email + LinkedIn Sequences)
→ Engagement / Reply Management (Approval Status = "Responded")
→ Zoho CRM Lead Creation (Lead_Status = "Engaged") & Sync Back (Zoho Record ID, Zoho Sync Status = "Synced")
...
- Apollo.io — lead acquisition, prospect research, contact intelligence, and native outbound email & multichannel sequence execution
- Zoho CRM — system of record for engaged leads, opportunities, and deals
- Google Antigravity / Gemini — AI reasoning, workflow orchestration, deep email/LinkedIn personalization, and Skills (with Apollo & Zoho CRM MCP integrations)
- Zoho Cliq — internal communication / notifications where applicable

Saleshandy and Instantly are NOT active sending platforms.

Any Saleshandy- or Instantly-related information is historical/archive information only.

Never:
- create or modify external Saleshandy campaigns
- update Saleshandy sequence IDs
- reference Saleshandy as the current sending platform
- use historical Saleshandy or Instantly mappings as current configuration
...
→ PTB Scoring & Company Trigger
→ Campaign Selection & Target Segment / Outreach Angle
→ Antigravity Personalization (Personalised Email + Pain Point + LinkedIn)
→ Apollo Contact Custom Fields Enrichment (Approval Status = "New", Zoho Sync Status = "Not Synced")
→ BDM Human Approval in Apollo (Approval Status = "Approved for Outreach")
→ Apollo Sequences Enrollment & Multichannel Outreach (Email + LinkedIn)
→ Reply / Engagement Tracking (Approval Status = "Responded")
→ Zoho CRM Lead Creation (Lead_Status = "Engaged") & Sync Back (Zoho Record ID, Zoho Sync Status = "Synced")
→ Opportunity
→ Proposal Preparation
→ Proposal Generation
...
Zoho CRM is the system of record for engaged leads, opportunities, and deal business data.

**Response-Triggered Lead Creation Rule:**
Leads are added to Zoho CRM **ONLY when a response is received** from outreach. Cold, uncontacted leads reside exclusively in Apollo with their enriched custom fields. Zoho CRM is never cluttered with uncontacted or unresponsive cold prospects.

When a reply or response is detected by `apollo-reply-tracker`:
1. `crm-update` verifies that the contact does not already exist in Zoho CRM (search by email).
2. `crm-update` creates the lead in Zoho CRM with `Lead_Status = "Engaged"`, populates the business-process and intelligence fields, and retrieves the generated Zoho `Record ID`.
3. `crm-update` writes the `Zoho Record ID` back to the Apollo contact's custom field and updates `Zoho Sync Status = "Synced"`.

`crm-update` is the single Skill responsible for business-process CRM writes.
...
Apollo Sequences is the current outbound multichannel platform.

Campaign and sequence configuration must be resolved through:

`context/campaigns.md` — LIVE CAMPAIGN REGISTRY section

Do not hard-code Apollo sequence IDs inside business logic where a
...
This allows Apollo sequence IDs to change without changing the
business logic.
...
A lead may enter an active Apollo Sequence only after the required human approval state
exists directly on the Apollo contact record (`Approval Status = "Approved for Outreach"`).
...
outreach/
    apollo_distribution.md
    apollo_reply_tracker.md
